...
# defined in ProfileLayout, generates the tree for us

def generateFigure(PF, sample, rank, output_base_name, output_path,file_type, plot_l1, scaling, output_dpi, use_profile, fw, fh, limit=None, limit_value=None):
    ...
    # define generic tree properties
    ts = TreeStyle()
    ...
    # T1 = TextFace("Taxy-Pro ", fsize=legend_fs)
    # T1.hz_align = True
    ...
    #     T2 = TextFace("Metaphyler", fsize=legend_fs)
    #     T2.hz_align = True
...
def main():
    ...
    argparser.add_argument('-i1', '--input_profile1', type=str, default='0', help='Second (optional) input taxonomic profile1')
    ...
    argparser.add_argument('-k', '--scaling', type=str, default='linear', help="Plot scaling (log, sqrt, power etc.")
    ...
    argparser.add_argument('-c', '--contrast', type=str, default='False', help="contrast mode for comparison with gold standard")
    # argparser.add_argument('-fir', '--input1', type=str,help="Name of the first input")
    ...
    input_file1 = params.input_profile1
    ...
    # Now the other file does everything 

    # ingest the profiles information
    PF = ProfilesLayout(input_file, input_file1,ground_truth, scaling, labels, layt, contrast, normalize=normalize, fs=fs, ls=ls, bm=bm, lw=lw, lsep=lsep)
